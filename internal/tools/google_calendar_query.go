...
func (t *GoogleCalendarQueryTool) Execute(ctx context.Context, params json.RawMessage) (*Result, error) {
	...
	if operation == "" {
		return &Result{Success: false, Error: "operation is required"}, nil
	}
	if operation != "list_calendars" && operation != "list_events" {
		return &Result{Success: false, Error: "unsupported operation; use list_calendars or list_events"}, nil
	}
...
var _ Tool = (*GoogleCalendarQueryTool)(nil)
